...
namespace HtmlRenderer.Core.Utils
{
    ...
    internal static class FontsUtils
    {
        ...
        /// </summary>
        private static readonly Dictionary<string, IFontFamily> _existingFontFamilies = new Dictionary<string, IFontFamily>(StringComparer.InvariantCultureIgnoreCase);
            ...
            if (font == null)
            {
                if (!_existingFontFamilies.ContainsKey(family))
                {
                    ...
                    if(_fontsMapping.TryGetValue(family, out mappedFamily))
                    {
                        ...
                        if(font == null)
                        {
                            font = CreateFont(htmlContainer, mappedFamily, size, style);
                ...
                if(font == null)
                {
                    font = CreateFont(htmlContainer, family, size, style);
                }
        ...
        public static void AddFontFamily(IFontFamily fontFamily)
        {
        ...
        private static IFont CreateFont(HtmlContainerInt htmlContainer, string family, float size, FontStyleInt style)
        {
            ...
            return _existingFontFamilies.TryGetValue(family, out fontFamily) ? g.CreateFont(fontFamily, size, style) : g.CreateFont(family, size, style);
        }
